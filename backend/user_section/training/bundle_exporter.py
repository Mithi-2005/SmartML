...
def export_user_bundle(task_type: str, user_id: str, dataset_name: str, model_path: str, preprocessor, status_tracker=None) -> Path:
    ...
    temp_bundle_dir_obj = None
    if azure_blob_helper.enabled:
        import tempfile
        temp_bundle_dir_obj = tempfile.TemporaryDirectory()
        base_dir = Path(temp_bundle_dir_obj.name)
    else:
    ...
    # Create zip using shutil.make_archive
    ...
    if zip_path:
        ...
        # Upload model and template artifacts to Azure Blob Storage if enabled
        try:
            if azure_blob_helper.enabled:
                azure_blob_helper.upload_file(zip_path, f"storage/{user_id}/templates/{task_type}/{zip_path.name}")
                if template_meta_path.exists():
                    azure_blob_helper.upload_file(template_meta_path, f"storage/{user_id}/templates/{task_type}/{template_meta_path.name}")
                if model_file.exists():
                    azure_blob_helper.upload_file(model_file, f"storage/{user_id}/models/{task_type}/{model_file.name}")
                if meta_path.exists():
                    azure_blob_helper.upload_file(meta_path, f"storage/{user_id}/models/{task_type}/{meta_path.name}")
        except Exception as azure_err:
            print(f"Warning: Failed to upload files to Azure Blob Storage: {azure_err}")
        ...
        # Cleanup: Delete the local zip file and template metadata if Azure is enabled!
        if azure_blob_helper.enabled:
            try:
                if zip_path.exists():
                    zip_path.unlink()
                ...
                print(f"Warning: Failed to cleanup zip/template files locally: {e}")

        # Clean up temporary directory object
        if temp_bundle_dir_obj:
            try:
                temp_bundle_dir_obj.cleanup()
            except Exception as e:
                print(f"Warning: Failed to cleanup temporary bundle directory object: {e}")
